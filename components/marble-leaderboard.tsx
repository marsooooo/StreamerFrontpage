...
import { fetchSheetNames, fetchSheetData } from "@/lib/api-client"
...
export default function MarbleLeaderboard({ onPlayerCountChange }: MarbleLeaderboardProps) {
  ...
  const notifiedCount = useRef<number | null>(null)

  const { data: availableSheets = [], isLoading: sheetsLoading } = useSWR("sheets-names", fetchSheetNames, {
    revalidateOnFocus: false,
    dedupingInterval: 60000,
  })

  // Set default sheet when available
  useEffect(() => {
  ...
  const {
    ...
    error,
  } = useSWR(selectedSheet ? `sheets-data-${selectedSheet}` : null, () => fetchSheetData(selectedSheet), {
    revalidateOnFocus: false,
    dedupingInterval: 60000,
  })

  // Notify parent of player count (only when changed)
  useEffect(() => {
    if (data.length > 0 && notifiedCount.current !== data.length) {
      notifiedCount.current = data.length
      onPlayerCountChange?.(data.length)
    }
  }, [data.length, onPlayerCountChange])

  const loading = sheetsLoading || dataLoading
